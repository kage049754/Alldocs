...
object DocxWriter {
    fun write(context: Context, uri: Uri, body: String) {
        ...
        ZipOutputStream(out).use { zip ->
            ...
            val paragraphs = body.split("\\n").joinToString("") { p -> "<w:p><w:r><w:t xml:space=\"preserve\">${escape(p)}</w:t></w:r></w:p>" }
            entry(zip, "word/document.xml", "<?xml version=\"1.0\" encoding=\"UTF-8\"?><w:document xmlns:w=\"http://schemas.openxmlformats.org/wordprocessingml/2006/main\"><w:body>$paragraphs<w:sectPr><w:pgSz w:w=\"12240\" w:h=\"15840\"/><w:pgMar w:top=\"1440\" w:right=\"1440\" w:bottom=\"1440\" w:left=\"1440\"/></w:sectPr></w:body></w:document>")
        }
    }
